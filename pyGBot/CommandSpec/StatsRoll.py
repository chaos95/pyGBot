...
def StatsRoll(bot, channel, user, args):
    sortdir = None
    verbose = None
    args = " " + args.lstrip(" ")
    ...
    if verbose:
        ...
        bot.replyout(channel, user, str(rolls))

    bot.replyout(channel, user, "Stats are: %s." % ', '.join(map(str, stats)))


def rollStats(sortdir):
